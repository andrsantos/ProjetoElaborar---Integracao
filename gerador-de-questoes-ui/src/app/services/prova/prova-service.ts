...
export class ProvaService {
  ...
  private readonly API_URL = 'http://localhost:8082/api/provas';
  private readonly API_URL_SALVAS = 'http://localhost:8082/api/provas-salvas';
  private readonly API_URL_TOPICOS = 'http://localhost:8082/api/topicos'; 
  private readonly API_QUESTOES_GERAR = 'http://localhost:8082/api/questoes/gerar'; 
  ...
  getConceitosPorDocumento(documentoId: string): Observable<string[]> {
    return this.http.get<string[]>(`http://localhost:8082/api/documentos/${documentoId}/conceitos`);
  }

  gerarProvaExpressa(payload: any): Observable<any> {
    return this.http.post<any>(`http://localhost:8082/api/provas/expressa`, payload);
  }
